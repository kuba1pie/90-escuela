  ...
  fetch('https://api.escuelajs.co/api/v1/products/'
    ...
    .then((response) => {
      if (response.status === 201) {
        ...
        store.statusButton = 'Successfully created product'
      }
      else {
        catchError()
      }
...
<template>
  <div id="c-EditItem" class="c-theForm card w-lg text-center flex flex-col mx-auto my-10 b-2 p-2">
    ...
    <div class="form-control">
      ...
      <Teleport to="body">
        <ConfirmModal :show="showModal" content="Create new item?" @close="showModal = false, store.status = '', store.statusButton = ''" @accept="saveItem(newItem)" />
      </Teleport>
    </div>
  </div>
</template>
